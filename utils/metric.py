...
def get_global_entity():
    ...
    hospital_db = load_json('data/WOZ/db/hospital_db.json')
    attraction_db = load_json('data/WOZ/db/attraction_db.json')
    hotel_db = load_json('data/WOZ/db/hotel_db.json')
    police_db = load_json('data/WOZ/db/police_db.json')
    restaurant_db = load_json('data/WOZ/db/restaurant_db.json')
    taxi_db = load_json('data/WOZ/db/taxi_db.json')
    train_db = load_json('data/WOZ/db/train_db.json')
    db_list = [hospital_db,attraction_db,hotel_db,police_db,restaurant_db,taxi_db,train_db]
    types =["address","area","name","phone","postcode","type", 
        "department","pricerange","stars","food", "taxi_colors",
        "taxi_types","arriveBy","day","departure","destination",
        "duration","leaveAt","price","trainID"]
    ...
    for db in db_list:
        for elem in db:
            for ty in types:
                if ty in elem: 
                    global_entity.add(normalize(str(elem[ty])).replace(" ","_") )
    global_entity.remove("?")
  
    return global_entity

def get_entity_by_domain():
    hospital_db = load_json('data/WOZ/db/hospital_db.json')
    attraction_db = load_json('data/WOZ/db/attraction_db.json')
    hotel_db = load_json('data/WOZ/db/hotel_db.json')
    police_db = load_json('data/WOZ/db/police_db.json')
    ...
    taxi_db = load_json('data/WOZ/db/taxi_db.json')
    train_db = load_json('data/WOZ/db/train_db.json')
    db_list = {"hospital":hospital_db,"attraction":attraction_db,"hotel":hotel_db,"police":police_db,"restaurant":restaurant_db,"taxi":taxi_db,"train":train_db}
    types =["address","area","name","phone","postcode","type", 
        "department","pricerange","stars","food", "taxi_colors",
        "taxi_types","arriveBy","day","departure","destination",
        "duration","leaveAt","price","trainID"]
    ...
    entity_by_domain = {}
    for domain, db in db_list.items():
        global_entity = set()
        for elem in db:
            for ty in types:
                if ty in elem: 
                    global_entity.add(normalize(str(elem[ty])).replace(" ","_") )
        if("?" in list(global_entity)):
          global_entity.remove("?")
        entity_by_domain[domain] = list(global_entity)
  
    return entity_by_domain
